...
根据前三个进入指定的文件夹，根据time拿到对应的日志文件，再检索出包含lotId和rule的日志


如果和用户对话，我第一步检索知识库SOP，给出用户操作建议，第二步检索需求知识库，给用户描述这块的需求，此时，如果用户不满意，我就需要用户提供env: serverIp:module:time: lotId:rule:，这几个指标，我根据这几个指标查询日志，或者用户告诉我日志是什么，我根据日志调用另外一个代码分析的agent。请帮我设计下怎么实现
